...
category: blog
excerpt: "ぼくがメンテナを務める Ruby の docx gem を一日で4回リリースし、ヘッダー・フッターの読み書き対応など溜まっていた機能・修正をまとめて取り込んだ話を紹介します。"
header:
...
ヘッダー・フッターが読み込めるようになったので、ブックマークのスキャン対象にも含めるようにしました。
なので、ヘッダー・フッター内のブックマークの前後にテキストを追加したりもできるようになっています。
...
そんな追加機能の`Paragraph#substitute` は、プレースホルダーの置換を、同じ段落内であっても複数のRunにまたがっていてもできるようにする機能です。
普通にdocxファイルを編集していると、気づくとRunが分割されているということはよくあることだったと思います。
そういった場合でも、プレースホルダーを置換できるようになったのです。
